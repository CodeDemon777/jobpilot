        ...
        for token in board_tokens:
            try:
                ...
                for job_data in data.get("jobs", []):
                    ...
                    loc_name = loc.get("city", "") + ", " + loc.get("region", "") if isinstance(loc, dict) else str(loc)
                    if location and location.lower() not in loc_name.lower():
                        continue
                    job_url = job_data.get("url", "")
                    desc = job_data.get("description", "")
                    skills = self._extract_skills(desc)
                    remote_status = "remote" if "remote" in loc_name.lower() else "onsite"
                    jobs.append(JobListing(
                        company=token.title(), title=title, location=loc_name,
                        remote_status=remote_status, required_skills=skills,
                        description=desc[:500], url=job_url, source="workable",
