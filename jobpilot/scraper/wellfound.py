...
class WellfoundScraper(BaseScraper):
    ...
    async def search(self, query: str, location: str = "", **kwargs) -> list[JobListing]:
        ...
        try:
            ...
            for card in soup.select("div.styles_jobList__lqjPr a, div[data-test=JobListing]"):
                title_el = card.select_one("h2, span.styles_title__dAoph")
                company_el = card.select_one("span.styles_companyName__mWtvj, span[data-test=company-name]")
                if not title_el:
                    continue
                ...
                jobs.append(JobListing(
                    company=company, title=title, location=location or "Remote",
                    remote_status="remote", required_skills=skills,
                    url=job_url, source="wellfound", tech_stack=skills,
