    ...
    async def search(self, query: str, location: str = "", **kwargs) -> list[JobListing]:
        ...
        try:
            ...
            for job_data in data.get("jobs", data.get("data", [])):
                ...
                company = job_data.get("company", {}).get("name", "Unknown") if isinstance(job_data.get("company"), dict) else job_data.get("companyName", "Unknown")
                ...
                job_url = job_data.get("url", f"https://cutshort.io/company/{job_data.get('slug', '')}")
                remote_status = "remote" if "remote" in loc.lower() else "onsite"
                jobs.append(JobListing(
                    company=company, title=title, location=loc,
                    remote_status=remote_status, required_skills=skills if isinstance(skills, list) else [],
                    description=desc[:500], url=job_url, source="cutshort",
                    tech_stack=skills if isinstance(skills, list) else [], application_url=job_url,
                ))
