...
class CareerPagesScraper(BaseScraper):
    ...
    async def search(self, query: str, location: str = "", **kwargs) -> list[JobListing]:
        ...
        for company, url in company_urls.items():
            try:
                search_url = f"{url}?q={query}" if "?" not in url else f"{url}&q={query}"
                response = await self._fetch(search_url, headers={
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
                })
                from bs4 import BeautifulSoup
                soup = BeautifulSoup(response.text, "html.parser")
                for card in soup.select("a[href*='job'], div[class*='job'], tr[class*='job'], li[class*='job']"):
                    ...
                    if query_lower and query_lower not in title.lower():
                        continue
                    href = card.get("href", "") or (title_el.get("href", "") if title_el else "")
                    if href and not href.startswith("http"):
                        href = f"{url.rsplit('/', 1)[0]}{href}"
                    loc = "Unknown"
                    skills = self._extract_skills(card.get_text())
                    jobs.append(JobListing(
                        company=company.title(), title=title, location=loc,
                        remote_status="unknown", required_skills=skills,
                        url=href, source="career_pages", tech_stack=skills,
