...
def test_match_skills_weight():
    ...
    assert (
        r2.skills_score > r1.skills_score
    ), "Job with matching skills should score higher"
...
def test_seed_and_match():
    ...
    assert results[0][0].company in (
        "Co1",
        "Co3",
    ), f"Expected Co1/Co3 first, got {results[0][0].company}"
    assert (
        results[-1][0].company == "Co2"
    ), f"Expected Co2 last, got {results[-1][0].company}"
...
def test_top_matches():
    ...
    for i in range(10):
        j = JobListing(
            ...
            url=f"http://t{i}.com",
            source="test",
            required_skills=["python"] if i < 5 else ["rust"],
            remote_status="remote",
            location="Remote",
            experience_years=2,
        )
